/*
    ...
    Purpose  : Present each customer's service configuration and derived
               bundle flags. Consumed by fct_customer_monthly_snapshot
               (via int_customer_monthly_360) and by dim_service_profile
               (which selects DISTINCT on the natural attribute set).

    service_profile_key is a stable surrogate hash over the full service
    attribute set. The same expression is used in dim_service_profile so
    keys reconcile automatically.
*/
